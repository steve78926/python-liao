...
def _profiling(start, sql=''):             #error: __   --> _
    t = time.time() - start
    if t > 0.1:
...
def create_engine(user, password, database, host='127.0.0.1', port=3306, **kw):
    ...
    for k, v in defaults.iteritems():           #error: k , ----> k,
        params[k] = kw.pop(k, v)
    params.update(kw)
...
class _ConnectionCtx(object):
    ...
    def __enter__(self):
        ...
        if not _db_ctx.is_init():
            _db_ctx.init()
            self.should_cleanup = True              #error: shound ----> should
        return self
...
class _TransactionCtx(object):
    ...
    def __enter__(self):
        ...
        if not _db_ctx.is_init():
            ...
            _db_ctx.init()
            self.should_close_conn = True               #error: shound  ----> should
...
def select(sql, *args):
    ...
    >>> insert('user', **u1)           #error: 缩进必须4个空格，否则报错
    1
    >>> insert('user', **u2)
    1
